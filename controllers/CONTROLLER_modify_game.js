...
const controller = {
    ...
    getGame: function(req, res){
        ...
        db.findOne(Game, {_id: id}, null, function(game){
            db.findMany(Item, {game_id: id}, null, function(items){
                res.render("pages/modify_game", {game: game, items: items, guest: req.session.guest});
            })
    ...
    modifyGame: function (req, res){
        ...
        db.findOne(User, {email: req.session.username}, '_id', function(creator){
            if (req.body.game_image)
                db.updateOne(Game, {_id: id}, {_id: id, title: title, description: description, game_image: game_image, genres: genres, time: time, creator: creator._id, num_attempts: 0});
            else
